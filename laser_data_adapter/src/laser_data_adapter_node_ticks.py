...
with open(file_laser_tstamps,'r') as tstamps_file:
    ...
    for item in range(0,len(tstamp)):
        ticks = int(tstamp[item]) #timestamps in ticks must be converted to unix epoch
        start = datetime.datetime(1,1,1)
        delta= datetime.timedelta(seconds=ticks/10000000)
        actual_date= start +delta
        secs=actual_date.timestamp()
        tstamp[item]=rospy.Time(secs)#turning the timestamp values to timestamp object
